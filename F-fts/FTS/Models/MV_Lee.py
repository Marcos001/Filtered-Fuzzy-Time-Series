...
class STFMV_Convencional_Lee:
    ...
    def __init__(self):
        ...
        ------
         ...
         - Uses the rules and defuzzification model proposed by Lee 2009
         - It performs the prediction of the numerical value only, 
         since it knows the linguistic term.
        '''
    ...
    def cronological_recurrence_rules(self):
        '''
        weigthed rules
        ...
        for i in range(self.X.shape[0]-2):
            ...
            self.flrg[antecedente].append(consequente)
    ...
    def defuzzification_LEE(self, consequentes):
        ...
        mid_points = []
        for term_cons in consequentes: mid_points.append(float(self.df_fuzzy_sets[term_cons]))
    ...
    def MidPoint_Weighted(self, ):
        ...
        for i in range(self.X.shape[0]-1):
            ...
            if len(self.flrg[antecedente]) == 0: y_pred.append(self.defuzzification_LEE([antecedente]))
            else: y_pred.append(self.defuzzification_LEE(self.flrg[antecedente]))
    ...
    def fit(self, X, k, log=False):
        ...
        # create rules
        self.cronological_recurrence_rules()

        if log:
            self.summary()


    def predict(self, plot=True, points=True, SM=True):
        '''
        Make forecasting
        Parameters
        ----------
            plot: boolean (optional)
              - plot graph of forecasting
            points: boolean (optional)
             - trace points more line in plot
            SM: boolean (optional)
             - print metrics forecasting , MSE, MAE, RMSE, DTW, MAPE
        Return
        ------
        validation metrics of the forecasting
          - MAPE, MAE, MSE, RMSE, DTW
        '''
        y_true = self.X[1:, 0]
...
class STFMV_Auto_Lee:
    ...
    def __create_fuzzy_sets(self):
        ...
        # associate the data - fuzzy or crisp
        self.clusters = {}
        conjuntos_y = {}
    ...
    def fuzzify(self, value, term=False, verbose=False):
        ...
        pertinence (float), linguist term (string)
        }
        """
        perts = []
    ...
    def cronological_recurrence_rules(self):
        ...
        self.flrg = {}
        for termo in set(self.mf['term']): self.flrg[termo] = []
    ...
    def fit(self, ts_x, ts_y, C=7, train=1, association='fuzzy',  log=False):
        ...
        if train == 1:
            self.partition = self.ts_array.shape[0]
        else:
